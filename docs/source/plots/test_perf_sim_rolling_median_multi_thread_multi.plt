...
# set logscale y2
set y2label "Ratio Multi/Single"
set y2range [0:5]
# set y2range [1:1e9]
set y2tics
...
plot "dat/test_perf_sim_rolling_median_multi_thread_single.dat" using 3:(1e6 * ($5 - $6) * $4 / $9):(1e6 * $7 * $4 / $9):(1e6 * $8 * $4 / $9):(1e6 * ($5 + $6) * $4 / $9) t "Single-threaded" with candlesticks whiskerbars 0.5, \
    ...
    "dat/test_perf_sim_rolling_median_multi_thread_multi.dat" using 3:($7 * $4 / $9 / (10 * 0.000020417 / 200)) axes x1y2 t "Ratio Multi/Single [right]" with lines lw 2
