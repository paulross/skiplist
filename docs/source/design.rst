...
This is a generic skip list implementation for any type T that is comparable.
The only restriction on the size of this skip list is the available memory. There are no configuration requirements (maximum size, capacity etc.).
